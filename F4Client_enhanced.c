...
// ANSI color codes for enhanced terminal graphics
  	...
  	stampaCampo();
   	...
   	//Il server restituisce un numero negativo quando la partita finisce. -1 e -2 indicano rispettivamente chi ha vinto dei due.
        if(memoria_condivisa->mossa==-1){
	  	if(giocatore == 1)
	  		printf(BOLD GREEN_COLOR "🎉 CONGRATULATIONS! YOU WON! 🎉" RESET_COLOR "\n");
	  	else
	  		printf(BOLD RED_COLOR "😞 YOU LOST! Better luck next time!" RESET_COLOR "\n");
		...
		if(memoria_condivisa->mossa==-2){  
			...
			printf(BOLD YELLOW_COLOR "🤝 GAME ENDED IN A DRAW!" RESET_COLOR "\n");
		}
	}
	printf("\n");
}
...
void attesaTurno(){
	...
	system("clear");
	printf(CYAN_COLOR "✓ Move inserted: Column %d" RESET_COLOR "\n", memoria_condivisa->mossa+1);
	printf(YELLOW_COLOR "⏳ Waiting for opponent's move..." RESET_COLOR "\n");
	semOp(semid, 0, 1);
	semOp(semid, giocatore,-1);
}
...
void stampaCampo(){
    ...
    for (int j = 0; j < memoria_condivisa->colonne; j++) {
        printf(BOLD YELLOW_COLOR " %d " RESET_COLOR, j + 1);
    }
    printf("\n");
    ...
    for (int j = 0; j < memoria_condivisa->colonne; j++) {
        printf("───");
        if (j < memoria_condivisa->colonne - 1) printf("┬");
    }
    printf("┐\n");
    ...
    for (int i = 0; i < memoria_condivisa->righe; i++) {
        printf("  │");
        for (int j = 0; j < memoria_condivisa->colonne; j++) {
            char piece = memoria_condivisa->campo[i][j];
            
            // Color coding for different pieces
            if (piece == 'X') {
        ...
        if (i < memoria_condivisa->righe - 1) {
            printf("  ├");
            for (int j = 0; j < memoria_condivisa->colonne; j++) {
                printf("───");
                if (j < memoria_condivisa->colonne - 1) printf("┼");
...
void mossa(){
	...
	printf(BOLD GREEN_COLOR "► It's your turn! (%c" RESET_COLOR, memoria_condivisa->gettone);
	if (memoria_condivisa->gettone == 'X') {
		printf(RED_COLOR " ● " RESET_COLOR);
	} else {
		printf(BLUE_COLOR " ● " RESET_COLOR);
	}
	printf(BOLD GREEN_COLOR ")" RESET_COLOR "\n");
	
	do{	
		stato = false;	
		printf(YELLOW_COLOR "Enter column number (1-%d): " RESET_COLOR, memoria_condivisa->colonne);
		if(scanf("%d", &mossa)!=1){
				stato = true;
				while (getchar()!='\n');
				fflush(stdout);
				printf(RED_COLOR "⚠️  Invalid input! Please enter a number" RESET_COLOR "\n");
		}else{
			if(mossa<=0 || mossa>memoria_condivisa->colonne){
				printf(RED_COLOR "⚠️  Invalid move! Please choose a column between 1 and %d" RESET_COLOR "\n", memoria_condivisa->colonne);
				stato = true;
			}else{
				if(memoria_condivisa->campo[0][mossa-1]!=' '){
				 	printf(RED_COLOR "⚠️  Column is full! Choose another column" RESET_COLOR "\n");
				 	stato = true;
				}
			}
		}
